...
function connectws() {
  ...
  ws.onmessage = (webSocketMessage) => {
    ...
    switch (messageBody.function){
      ...
      case "ChatMessages": ChatHistoryLoad(messageBody.messages); break;
...
function ChatHistoryLoad(messages) {
  for(let i=0;i<messages.length;i++) {
    document.getElementById("chatwindow").innerHTML += messages[i] + '<br>';
  }
}
